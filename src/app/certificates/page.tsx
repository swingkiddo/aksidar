...
function CertificatesContent() {
  ...
  const openCert = (cert: { name: string; image: string }, index: number) => {
  ...
  return (
    <>
      ...
      <Dialog open={!!selectedCert} onOpenChange={(open) => !open && closeCert()}>
        <DialogContent className="max-w-4xl w-full bg-cream border-green-mist/40">
          ...
          <div className="flex justify-between items-center pt-4 border-t border-green-mist/30">
...
export default function CertificatesPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-cream flex items-center justify-center"><div className="animate-pulse text-green-mid">Загрузка...</div></div>}>
      <CertificatesContent />
    </Suspense>
  );
}
